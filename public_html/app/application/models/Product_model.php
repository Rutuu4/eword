...
class Product_model extends CI_Model
{
    ...
    function product_list($params)
    {
        ...
        $query = $this->db->query("
                ...
                ".(!empty($sqla_ddquery_left_city)?$sqla_ddquery_left_city:'')."
    ...
    function product_search_list($params){
        extract($params);
        ...
        $query = $this->db->query("
                ...
                FROM 
                    mp_product 
                WHERE 
                    mp_product.status = 1 and mp_product.profile_select_id = 1 AND 
                    (mp_product.product_name LIKE '%".$search_text."%' OR mp_product.mrp_price LIKE '%".$search_text."%' OR mp_product.sell_price LIKE '%".$search_text."%')
                GROUP BY
                    mp_product.id  
                ORDER BY 
                    rand()
