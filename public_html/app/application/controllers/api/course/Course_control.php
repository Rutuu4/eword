...
class Course_control extends REST_Controller
{
    ...
    public function course_details_post()
    {
        ...
        if (!empty($chkUser)) {
            ...
            $productList = $this->General_model->get_query_data($params);

            if (!empty($productList)) {
                for ($i = 0; $i < count($productList); $i++) {
                    $productList[$i]['pdf_file'] = check_file_exists($productList[$i]['pdf_file']);

                    // 🔹 Get related course videos
                    $videoParams = array(
                        'table'       => TBL_COURSE_VIDEOS . ' as course_videos',
                        'wherestring' => "course_videos.course_details_id = " . $productList[$i]['id'],
                    );
                    $videos = $this->General_model->get_query_data($videoParams);

                    // Attach videos (array of name + link)
                    if (!empty($videos)) {
                        $videoArr = [];
                        foreach ($videos as $vid) {
                            $videoArr[] = [
                                'name' => $vid['video_name'],       // make sure `name` column exists in `course_videos`
                                'video_link' => $vid['video_link'],
                                'display_order' => $vid['display_order']
                            ];
                        }
                        $productList[$i]['videoAndDescription'] = $videoArr;
                    } else {
                        $productList[$i]['videoAndDescription'] = [];
                    }
                }
