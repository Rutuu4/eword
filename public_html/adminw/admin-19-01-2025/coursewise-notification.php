        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            -->
                    <div class="box-body">
              <h4>
              Manage Coursewise Notification List (Last 30 Days)
               <a href="master/create-coursewise-notification.php" class="btn btn-primary pull-right" >Create Notification</a>
              </h4>
              <?php
          ...
          <div class="box">
            <div class="box-body table-responsive">
              <table id="datatable" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php
                  $qry="SELECT mobile_notification.*,m_main_courses.name AS main_course_name,GROUP_CONCAT(courses_details.name ORDER BY courses_details.id) AS course_name_list FROM mobile_notification 
                  left join m_main_courses on m_main_courses.id=mobile_notification.main_course_id
                  LEFT JOIN courses_details  ON  FIND_IN_SET(courses_details.id, mobile_notification.course_ids)
                  WHERE mobile_notification.id>0 and mobile_notification.notification_type=1";
                  if(!empty($fromdate) AND  !empty($todate)){  $qry .= " && mobile_notification.datee between '$fromdate' and  '$todate'"; }
                  if(empty($fromdate) or  empty($todate)){ $qry .= " && mobile_notification.datee >= DATE_SUB(CURDATE(),INTERVAL 30 DAY)"; }  
               ...
               <tr>
                  ...
                  <td>
                    <!-- <a href="master/edit-notification.php?key=<?=base64_encode($row['id'])?>" class="btn btn-warning"><i class="fa fa-edit"></i> Edit</a> -->
