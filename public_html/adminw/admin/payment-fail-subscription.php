      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
              <h4>
               Payment Fail Subscription List 
              </h4>
              <?php
          ...
          <div class="box">
            <div class="box-body table-responsive">
                           Payment Fail List Details (Last 30 Days)
            ...
            where package_subscription_order.payment_gateway_success_status=0 and registration.id>0";
          if(!empty($fromdate) AND  !empty($todate)){  $qry .= " && package_subscription_order.date between '$fromdate' and  '$todate'"; }
           ...
           $qry .= "group by registration.id  order by package_subscription_order.date desc";
           $result=$conn->query($qry);
                  $i = 1;
                  while($row = $result->fetch_array()){

$register_id=$row['register_id'];
$qry_chk="SELECT registration.id FROM package_subscription_order 
LEFT JOIN registration ON package_subscription_order.user_id = registration.id
where package_subscription_order.payment_gateway_success_status=1 and package_subscription_order.subscription_expire_date>'$date' and registration.id='$register_id'";
$result_chk=$conn->query($qry_chk);
$rowcount_chk=mysqli_num_rows($result_chk);





                      $status = "<span class='badge bg-red'>Fail</span>";
...
if($rowcount_chk==0)
{
                ...
                <tr>
                  ...
                  <td><?=$status;?></td>
                 <td><a href="master/edit-payment-fail-subscription.php?key=<?=base64_encode($row['id'])?>" class="btn btn-warning"><i class="fa fa-eye"></i></a></td>
                </tr>
                <?php }  } ?>
              </table>
            </div>
