...
<body class="<?= $bodyclass ?>">
    <?php
    if (empty($_GET['refer_username'])) {
        echo "<a href='index.php'>Go Back</a>";
        exit;
    }

    $refer_username = mysqli_real_escape_string($conn, $_GET['refer_username']);

    // Get main user info
    $qry_user = "SELECT fullname, username 
             FROM registration 
             WHERE username = '$refer_username' 
             LIMIT 1";

    $res_user = $conn->query($qry_user);
    $user = $res_user->fetch_array();

    // Get referred users
    $qry = "SELECT * FROM registration 
        WHERE refer_username = '$refer_username'";
                                        ...
                                        <table id="example1" class="table table-bordered table-striped" style="margin-bottom:10px;">
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                    <tr>
                                                        <td><?= $i; ?></td>
                                                        <td><?= date("d/m/Y", strtotime($row2['datee'])); ?></td>
                                                        <td><?= $row2['username']; ?></td>
                                                        <td><?= $row2['fullname'] ?: '-'; ?></td>
                                                    </tr>
                                                <?php } ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', ".updatecartproduct", function() {
                ...
                var dis_product_weight = 0,
                    dis_product_bori = 0,
                    dis_total_product_weight = 0,
                    dis_total_product_bori = 0;
            ...
            $("#example1").on('click', '.remove-row', function() {
        ...
        $(document).on('click', '.product_delete', function(e) {
            ...
            if (purchase_invoice_txn_id > 0) {
                $.ajax({
                    ...
                    data: {
                        purchase_invoice_txn_id: purchase_invoice_txn_id
                    },
                    dataType: 'json',
                    success: function(data) {
