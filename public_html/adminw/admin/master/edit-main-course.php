...
{  
  ...
  $isExtra=mysqli_real_escape_string($conn,$_POST['isExtra']);
  $display_order=mysqli_real_escape_string($conn,$_POST['display_order']);

  $qury1="Update m_main_courses set name='$name',video_link='$video_link', status='$status',isExtra='$isExtra',display_order='$display_order'  where id='$id'";          
  $sq1=$conn->query($qury1);

  if(mysqli_affected_rows($conn)>=1){  header("location:../manage-main-course.php"); }
...
if(!empty($_GET['key'])){
  ...
  $qry="SELECT * FROM m_main_courses WHERE id='$id'";  
        ...
        <div class="box-body">
          <form action="" method="POST" id="" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="form-group">
                     ...
                     <div class="form-group">
                      <label class="col-sm-2">Sub Course Available :</label>
                      <div class="col-sm-6">
                        <div class="col-sm-3 col-xs-6">
                          <label>
                          <input name="isExtra" type="radio" value="0" <?php if($isExtra!=1){  ?>checked="checked"<?php } ?>  />
                          No </label>
                        </div>
                        <div class="col-sm-3 col-xs-6">
                          <label>
                          <input name="isExtra" type="radio" value="1" <?php if($isExtra==1){  ?>checked="checked"<?php } ?>  />
                          Yes</label>
                        </div>
