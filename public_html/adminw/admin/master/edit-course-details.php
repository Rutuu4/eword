...
$qury1="UPDATE courses_details SET main_courses_id='$main_courses_id',extra_course_id='$extra_course_id',name='$name',video_link='$video_link',details='$details',pdf_file='$pdf_file',status='$status' where id='$id'";
 $sq1 = $conn->query($qury1); 

  if(mysqli_affected_rows($conn)>=1){  header("location:../manage-course.php"); }
   ...
   <div class="form-group">
                      ...
                      <div class="col-sm-8">
                           ...
                           <select name="extra_course_id"  id="extra_course_id"  class="form-control select2"  required>
                            ...
                            {
                              ?>
                              <option <?php if($row['extra_course_id']==$rowb['id']){ echo "selected"; } ?> value="<?=$rowb['id'];?>"> <?=$rowb['name'];?> </option>
                              <?php } ?>
                                  </select>
                      </div>
                    </div>  
