...
/*  $qury1="UPDATE mobile_notification SET title='$title',mesage='$mesage',img='$photos',details='$details',status='$status',website_link='$website_link',main_course_id='$main_course_id',course_ids='$course_ids' WHERE id='$id'";          
  $sq1=$conn->query($qury1);*/

  if(mysqli_affected_rows($conn)>=1){  header("location:../coursewise-notification.php"); }
...
if(!empty($_GET['key'])){
  ...
  $main_course_id=$row['main_course_id'];
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>View Coursewise Notification Details</h1>
    </section>
          ...
          <form action="" method="POST" id="" class="form-horizontal" enctype="multipart/form-data">
            <input name="h1" type="hidden" id="h1" value="1" />
             <input name="id" type="hidden" id="id" value="<?=$id;?>" />
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="title"  id="title" placeholder="Enter  Title" value="<?=$row['title'];?>"  disabled>
              </div>
            </div>
             ...
             <div class="form-group">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="mesage"  id="mesage" placeholder="Enter  mesage" value="<?=$row['mesage'];?>"   disabled>
              </div>
            </div> 
            ...
            <?php if(!$qm1==""){ ?>               
             ...
             <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    <textarea class="form-control textarea" placeholder="Description" style="width: 100%; height: 250px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" name="details" id="details" disabled><?=$row['details'];?></textarea>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="website_link"  id="website_link" placeholder="Enter Website Link " value="<?=$row['website_link'];?>" disabled>
              </div>
            </div>
                 <div class="form-group">
                      ...
                      <div class="col-sm-8">
                           ...
                           <select name="main_course_id"  id="main_courses_id"  class="form-control" disabled >
                            ...
                            {
                              ?>
                              <option <?php if($row['main_course_id']==$rowb['id']){  echo"selected"; } ?>   value="<?=$rowb['id'];?>"> <?=$rowb['name'];?> </option>
                              <?php } ?>
                                  </select>
                      </div>
                    </div>
    ...
    <div class="col-sm-8 row">
        <?php 
            $sql_fv = "SELECT id, name FROM courses_details WHERE status=1 AND main_courses_id='$main_course_id'";
        ...
        ?>
            <div class="col-sm-4">
                <div class="checkbox">
                    <label>
                        <input type="checkbox" name="course_ids[]" <?php $fam_val=explode(",",$course_ids);  for($fs=0;$fs<count($fam_val);$fs++) { if($row_fv['id'] == $fam_val[$fs]) { ?> checked <?php } } ?> value="<?=$row_fv['id'];?>" disabled> <?=$row_fv['name'];?>
                    </label>
                </div>
            </div>
        <?php } ?>
