...
$year_id=mysqli_real_escape_string($conn,$_POST['year_id']);
  ...
  $qry_chk="SELECT MAX(id) AS max_id  FROM cut_off_details";    
  $result_chk=$conn->query($qry_chk);
  $row_chk=$result_chk->fetch_array();
  $display_order=$row_chk['max_id']+1;

  $qury1="INSERT INTO cut_off_details(user_id, create_datetime, year_id, name, short_details, details, pdf_file, status,display_order) VALUES ('$login_id','$create_datetime','$year_id','$name','$short_details','$details','$pdf_file','$status','$display_order')";          
