        ...
        if (($handle = fopen($filename, "r")) !== false) {
            ...
            { 
                while (($data = fgetcsv($handle)) !== false) {
                    $exrtacoursename = mysqli_real_escape_string($conn, trim($data[0]));
                    $name = mysqli_real_escape_string($conn, trim($data[1]));
                    $video_link = mysqli_real_escape_string($conn, trim($data[2]));
                          ...
                          $rowcount_subcourse = mysqli_num_rows($result_subcourse);
                           if ($rowcount_subcourse > 0)
                           {
                                ...
                                while($row_subcourse = $result_subcourse->fetch_array())
                                {
                                  ...
                                  $qry_chk="SELECT * FROM courses_details WHERE name SOUNDS LIKE '$name' AND extra_course_id='$extra_course_id'";
                                  ...
                                  if($rowcount>0)
                                  {
                                    ...
                                    $qry_update = "UPDATE courses_details 
                                       SET video_link = '$video_link' 
                                       WHERE name IN (
                                           SELECT name FROM (SELECT * FROM courses_details) AS temp 
                                           WHERE name SOUNDS LIKE '$name'
                                       ) and extra_course_id='$extra_course_id'";
                                    $sql_update = $conn->query($qry_update);   
                              }
                              if($found_course!=1)
                              {
            ...
            <div class="box-content">
              <div class="box box-info col-md-6">
                <div class="box-header with-border">
                  <h3 class="box-title"><i class="fa fa-pencil-square-o"></i> UPDATE  COURSE VIDEO LINK CSV </h3>
                </div>
                     <div class="box-body">
                      ...
                      <div class="col-sm-1">
                        ...
                        <div class="col-sm-1">
                          <a href="../../download-update-course-video-link.csv" download="download-update-course-video-link.csv" class="btn btn-success" download role="button">Dwonload Demo File</a>
                        </div>
                    </div>
