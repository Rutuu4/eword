...
if ($_POST['h101'] == 1) {
    if ($_GET['id'] != "") {
        ...
        try {
            // Delete related records from project_and_internship_courses
            $conn->query("DELETE FROM project_and_internship_courses WHERE project_and_internship_id = '$id'");
           

            // Now delete the record from the project_and_internship table
            $conn->query("DELETE FROM project_and_internship WHERE id = '$id'");
