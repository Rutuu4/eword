        ...
        if (($handle = fopen($filename, "r")) !== false) {
            ...
            if ($header && !empty($header[0])) { // Ensure header row is valid
                while (($data = fgetcsv($handle)) !== false) {
                    ...
                    $qry_chk="SELECT id FROM courses_details where name='$name' and main_courses_id='$main_courses_id' and extra_course_id='$extra_course_id'";
                    $result_chk=$conn->query($qry_chk);
                    $rowcount=mysqli_num_rows($result_chk);
                    if ($rowcount> 0) 
                    {
                        $error_msg[] = 'This Course Already Exists';
                        $error_sku[] = $name;
                    } 
                    else 
                    {
