...
$website_links=mysqli_real_escape_string($conn,$_POST['website_links']);
  ...
  $qry_chk="SELECT MAX(id) AS max_id  FROM scholarship_loan_details";    
  $result_chk=$conn->query($qry_chk);
  $row_chk=$result_chk->fetch_array();
  $display_order=$row_chk['max_id']+1;


  $qury1="INSERT INTO scholarship_loan_details(user_id, create_datetime, name, short_details, details, pdf_file, website_links, status,display_order) VALUES ('$login_id','$create_datetime','$name','$short_details','$details','$pdf_file','$website_links','$status','$display_order')";          
        ...
        <div class="box-body">
          <form action="" method="POST" id="" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="website_links"  id="website_links" placeholder="Enter Website Link " value="<?=$row['name'];?>"  required>
              </div>
            </div>
