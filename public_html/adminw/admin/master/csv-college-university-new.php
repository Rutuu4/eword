<?php 
  include("../../database.php"); 
   $error_msg=[]; 
if($_POST['h1']==1 and $_POST['college_university_type_id'] > 0)
{
              ...
              { 
                      ...
                      if(!empty($name))
                      {
                                  ...
                                  while($i<$total_array-1)
                                  {
                                        ...
                                        $website_link=$data[1];
                                        $city=trim($data[2]);
                                        $course_list=trim($data[3]);

                                        $qry_chk="SELECT id FROM college_university_details where name='$name'";
...
if (!empty($course_list)) {
    $course_names = explode("-", $course_list);
    $placeholders = "'" . implode("','", array_map('strtolower', $course_names)) . "'"; 
    $sql1 = "SELECT id FROM courses_details WHERE LOWER(name) IN ($placeholders)";
    ...
    while ($row1 = $result1->fetch_assoc()) 
    {
        ...
        $matching_ids1[] = $row1['id'];
    }

    $course_ids_temp = !empty($matching_ids1) ? implode(",", $matching_ids1) : '';

    $sq3="SELECT id FROM courses_details WHERE id IN($course_ids_temp) group by courses_details.name";
    $result3= $conn->query($sq3);
    $matching_ids = [];
    while ($row3 = $result3->fetch_assoc()) 
    {
        $matching_ids[] = $row3['id'];
    }
     $course_ids = !empty($matching_ids) ? implode(",", $matching_ids) : '';
    ...
    $course_ids = '';
                                                  ...
                                                  $insert_array[] ="('$login_id','$create_datetime','$college_university_type_id','$name','$city_id','$website_link','1','$course_ids','$course_name_list')";
                                              }
                                              else
                                              {
                                                $error_msg[]='This City Not Exists';
                                                $error_sku[]=$data[2];
      ...
      if(!empty($insert_array))
      {
         ...
         $qury2="INSERT INTO college_university_details(user_id, create_datetime, college_university_type_id,name, city_id,website_link, status,course_ids,course_name_list)
            ...
            <div class="box-content">
              <div class="box box-info col-md-6">
                <div class="box-header with-border">
                  <h3 class="box-title"><i class="fa fa-pencil-square-o"></i> CREATE COLLEGE & UNIVERSITY WITH CSV </h3>
                </div>
                     <div class="box-body">
                      ...
                      <div class="col-sm-2">
                        ...
                        <div class="col-sm-1">
                          <a href="../../download-college-university.csv" download="download-college-university.csv" class="btn btn-success" download role="button">Dwonload Demo File</a>
                        </div>
                    </div>
