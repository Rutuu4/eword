...
{  
  ...
  $chat_course_id=mysqli_real_escape_string($conn,$_POST['chat_course_id']);
  $name=mysqli_real_escape_string($conn,$_POST['name']);
  $status=mysqli_real_escape_string($conn,$_POST['status']);
  $member_limit=500;
  $is_hide=mysqli_real_escape_string($conn,$_POST['is_hide']);


  $qury1="UPDATE chat_room_group SET chat_course_id='$chat_course_id',name='$name',status='$status',member_limit='$member_limit',is_hide='$is_hide'  where id='$id'";          
  $sq1=$conn->query($qury1);

  if(mysqli_affected_rows($conn)>=1){  header("location:../manage-chat-room-group.php"); }
...
if(!empty($_GET['key'])){
  ...
  $qry="SELECT * FROM chat_room_group WHERE id='$id'";  
            ...
            <input name="h1" type="hidden" id="h1" value="1" />
             ...
             <div class="form-group">
                      ...
                      <div class="col-sm-8">
                           ...
                           <select name="chat_course_id"  id="chat_course_id"  class="form-control select2"  required>
              ...
              <div class="form-group">
                ...
                <div class="col-sm-6">
                   ...
                   <div class="col-sm-3 col-xs-6">
                       <label><input name="is_hide" type="radio" value="0" <?php if($is_hide!=1){  ?>checked="checked"<?php } ?> />Show</label>
                   </div>
                   <div class="col-sm-3 col-xs-6">
                       <label><input name="is_hide" type="radio" value="1" <?php if($is_hide==1){  ?>checked="checked"<?php } ?> />Hide</label>
                   </div>
                </div>
            </div>
