...
{  
  ...
  $qry_chk="SELECT MAX(id) AS max_id  FROM website_list";    
  $result_chk=$conn->query($qry_chk);
  $row_chk=$result_chk->fetch_array();
  $display_order=$row_chk['max_id']+1;

  $qury1="insert into website_list(id,name,website_link,status,display_order)values(NULL,'$name','$website_link','$status','$display_order')";
  
  // echo $qury1;         
  $sq1 = $conn->query($qury1);  
  $last_idd=mysqli_insert_id($conn);
  if(!$last_idd==""){ header("location:../manage-website.php"); }
