...
if ($_POST['h101'] == 1) {
    if ($_GET['id'] != "") {
        ...
        try {
            // Delete related records from tuition_and_training_courses
            $conn->query("DELETE FROM tuition_and_training_courses WHERE tuition_and_training_id = '$id'");
           

            // Now delete the record from the tuition_and_training table
            $conn->query("DELETE FROM tuition_and_training WHERE id = '$id'");
