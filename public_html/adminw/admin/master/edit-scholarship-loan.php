...
$website_links=mysqli_real_escape_string($conn,$_POST['website_links']);

   $display_order=mysqli_real_escape_string($conn,$_POST['display_order']);

$qury1="UPDATE scholarship_loan_details SET name='$name',short_details='$short_details',details='$details',pdf_file='$pdf_file',website_links='$website_links',status='$status',display_order='$display_order'  where id='$id'";
 $sq1 = $conn->query($qury1); 

  if(mysqli_affected_rows($conn)>=1){  header("location:../manage-scholarship-loan.php"); }
...
if(!empty($_GET['key'])){
  ...
  $qry="SELECT * FROM scholarship_loan_details WHERE id='$id'";  
          ...
          <form action="" method="POST" id="" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="form-group">
             ...
             <div class="form-group">
              ...
              <div class="col-sm-8">
                <input type="text" class="form-control" name="website_links"  id="website_links" placeholder="Enter Website Link " value="<?=$row['website_links'];?>"  required>
              </div>
            </div>
             <?php if(!$qm1==""){?>   
                    <div class="form-group">
                      ...
                      <div class="col-sm-1">
                       <a button class="btn btn-danger btn-sm" onClick="window.open('master/delete-scholarship-loan-pdf.php?id=<?=$id;?>',   'win1','width=950, height=800, menubar=no ,scrollbars=yes,top=50,left=100')"><i class="fa fa-times"></i></button></a>
                     </div>
                </div>
                 <?php  } ?>
