...
if ($_POST['h1'] == 1 && $_POST['main_courses_id'] > 0) {
    ...
    if ($_FILES["file"]["size"] > 0 && in_array($file_mime, $allowed_mimes)) {
        if (($handle = fopen($filename, "r")) !== false) {
            ...
            if ($header && !empty($header[0])) { // Ensure header row is valid
                while (($data = fgetcsv($handle)) !== false) {
                    ...
                    $qry_chk = "SELECT id FROM courses_details WHERE name = ?";
                    $stmt_chk = $conn->prepare($qry_chk);
                    $stmt_chk->bind_param("s", $name);
                    $stmt_chk->execute();
                    ...
                    if ($stmt_chk->num_rows > 0) {
                        ...
                        // Insert the data
                        $status = 1;
                        $qry_insert = "INSERT INTO courses_details (user_id, create_datetime, main_courses_id, name, video_link, details, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
                        $stmt_insert = $conn->prepare($qry_insert);
                        $stmt_insert->bind_param("issssss", $login_id, $create_datetime, $main_courses_id, $name, $video_link, $details, $status);
                        $stmt_insert->execute();
                    }
                ...
                <div class="box-header with-border">
                  <h3 class="box-title"><i class="fa fa-pencil-square-o"></i> CREATE COURSE WITH CSV </h3>
                    ...
                    <select class="form-control" name="main_courses_id"  id="main_courses_id" required="">
                          ...
                          <?php
                            $qrys = $conn->query("SELECT id,name FROM m_main_courses WHERE status = 1");
