...
<div class="wrapper"> 
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
              <h4>
               Outword Order Report-List (Last 30 Days)
              </h4>
              <?php
              ...
              <div class="form-group">
               ...
               <div class="col-lg-2" style="padding-top:24px">
                    ...
                    <a style="margin-left: 10px" href="outorder-csv-report.php?fromdate=<?php echo $fromdate; ?>&&todate=<?php echo $todate; ?>" class="btn btn-primary" role="button">Export CSV</a>
                  </div>
                <!-- /.input group -->
               </div>
              </form>
 ...
 <table id="datatable" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php
          $qry="SELECT product_order.id,product_order.created_date,product_order.invoice_id,user.name as create_name,m_customer.name AS customer_name,m_customer.mobile AS customer_phone,m_customer.email AS customer_email  FROM product_order 
            LEFT JOIN user ON product_order.user_id = user.id
            LEFT JOIN  m_customer ON product_order.customer_id = m_customer.id";
          if(!empty($fromdate) AND  !empty($todate)){  $qry .= " where product_order.created_date between '$fromdate' and  '$todate'"; }
           if(empty($fromdate) or  empty($todate)){ $qry .= " where product_order.created_date BETWEEN CURDATE()+1 - INTERVAL 30 DAY AND CURDATE()+1 "; }  
          ...
          $result=$conn->query($qry);
            ...
            <?php if($designaton_id==2)
                  { 
                          ...
                          { ?>
                              <td>
                              <a href="master/edit-order-qty.php?key=<?=base64_encode($row['id'])?>" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                              </td>
                    <?php } 
                          else
                          { ?>
            ...
            <?php } ?>
                   <td>
                    <a href="transaction-report.php?txn_type=<?php echo 2 ; ?>&&challan_id=<?php echo $row['id'];?>" class="btn btn-warning"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
                <?php } ?>
              </table>
